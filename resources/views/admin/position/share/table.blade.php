<table class="table custom-table text-nowrap text-truncate">
    ...
    <tbody>
        @php($i = 0)
        @php($no = $positions->currentPage() - 1 !== 0 ? strval($positions->currentPage() - 1) : '')
        @foreach ($positions as $position)
            <tr>
                @php($i++)
                @if ($i === 10)
                    @php($i = 0)
                    @php($no = $positions->currentPage())
                @endif
                ...
                <td>
                    <div class="d-flex">
                        <a type="button" class="btn-action btn-edit position_edit" data-id="{{ $position->id }}" data-toggle="tooltip" data-placement="bottom" title="Edit">
                        ...
                        <div id="position_edit_modal"></div>
                        <div class="d-inline" data-toggle="tooltip" data-placement="bottom" title="Delete">
                            <button type="button" class="btn-action btn-del p-0" data-toggle="modal" data-target="#btn_delete_position_{{ $position->id }}">
                        ...
                        <div class="modal fade custom-modal" id="btn_delete_position_{{ $position->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{ route('position_delete', ['id' => $position->id]) }}" class="btn btn-primary btn-reg">
                                            Yes
                                        </a>
                                    </div>
                                </div>
