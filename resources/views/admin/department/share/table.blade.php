<table class="table custom-table text-nowrap text-truncate">
    ...
    <tbody>
        @php($i = 0)
        @php($no = $departments->currentPage() - 1 !== 0 ? strval($departments->currentPage() - 1) : '')
        @foreach ($departments as $department)
            <tr>
                @php($i++)
                @if ($i === 10)
                    @php($i = 0)
                    @php($no = $departments->currentPage())
                @endif
                <th scope="row">{{ $no . $i }}</th>
                <td>{{ $department->name }}</td>
                <td>
                    <div class="d-flex">
                        <a type="button" class="btn-action btn-edit department_edit" data-id="{{ $department->id }}" data-toggle="tooltip" data-placement="bottom" title="Edit">
                        ...
                        <div id="department_edit_modal"></div>
                        <div class="d-inline" data-toggle="tooltip" data-placement="bottom" title="Delete">
                            <button type="button" class="btn-action btn-del p-0" data-toggle="modal" data-target="#btn_delete_department_{{ $department->id }}">
                        ...
                        <div class="modal fade custom-modal" id="btn_delete_department_{{ $department->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{ route('department_delete', ['id' => $department->id]) }}" class="btn btn-primary btn-reg">
                                            Yes
                                        </a>
                                    </div>
                                </div>
