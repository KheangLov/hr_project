<table class="table custom-table text-nowrap text-truncate">
    ...
    <tbody>
        @php($i = 0)
        @php($no = $groups->currentPage() - 1 !== 0 ? strval($groups->currentPage() - 1) : '')
        @foreach ($groups as $group)
            <tr>
                @php($i++)
                @if ($i === 10)
                    @php($i = 0)
                    @php($no = $groups->currentPage())
                @endif
                ...
                <td>
                    <div class="d-flex">
                        <a type="button" class="btn-action btn-edit group_edit" data-id="{{ $group->id }}" data-toggle="tooltip" data-placement="bottom" title="Edit">
                        ...
                        <div id="group_edit_modal"></div>
                        <div class="d-inline" data-toggle="tooltip" data-placement="bottom" title="Delete">
                            <button type="button" class="btn-action btn-del p-0" data-toggle="modal" data-target="#btn_delete_group_{{ $group->id }}">
                        ...
                        <div class="modal fade custom-modal" id="btn_delete_group_{{ $group->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{ route('group_delete', ['id' => $group->id]) }}" class="btn btn-primary btn-reg">
                                            Yes
                                        </a>
                                    </div>
                                </div>
